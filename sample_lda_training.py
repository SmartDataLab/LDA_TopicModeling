...
n_features = 2500
n_top_words = 40
n_topics = list(range(3, 10, 1))  # 话题数从5到100，根据perplexity决定
perplexityLst = [1.0] * len(n_topics)
originFilePath = "input.xls"  # 从xlsx文件中读取dframe
textPre_FilePath = "sample_input.txt"  # 保存文本预处理后的文件，避免重复预处理
...
pickle.dump(tf_vectorizer, open(tf_ModelPath, "wb"))
...
pickle.dump(lda_models, open(lda_ModelPath, "wb"))  # 按照ddhhmm进行命名
